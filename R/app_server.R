...
app_server <- function(input, output, session) {
  ...
  output$menu <- renderMenu({
    sidebarMenu(id = "smenu",
                ...
                menuItem("M-VIDIA",
                         helpText("multi-view proteomics"),
                         menuSubItem("View Extraction", "extract_view"),
                         menuSubItem("DEA", "mvidia_DEA"),
                         menuSubItem("Patient Diagnosis", "mvidia_pd"),
                         menuSubItem("Cell Clustering", "mvidia_clustering")
                ...
                menuItem("Help", helpText("tourial"),
                         ...
                         menuSubItem("MVIDIA_DEA", "Help4"),
                         menuSubItem("MVIDIA_PD", "Help5"),
                         menuSubItem("MVIDIA_Clustering", "Help6"),
  ...
  output$volc_MVIDIA<-renderUI({

    view_files<-input$view_dt_fold_dea

    logFC_mv=log2(as.numeric(input$FC))
    qval_mv = as.numeric(input$adjp)
    norm = input$normalization
    imput = input$imputation
    platform = input$rb_dea_plat
    acq = input$rb_dea_acq
    view_names = paste(input$rb_dea,collapse = ',')
    dea = input$DEA
    g1 = input$g1
    g2 = input$g2
    method = input$rb_dea_mth

    python_path_mv_dea = ''#input$python_path_fg_ens

    t1 = proc.time()[3]
    MVIDIA_DEA_out<-MVIDIA_DEA(view_files, logFC_mv, qval_mv, norm, imput,
                               platform, acq, view_names, dea, g1, g2,
                               python_path_mv_dea, method)
    t2 = proc.time()[3]
    output$DEA_result_MVIDIA<-renderText(paste0('MVIDIA DEA finished with time ',
                                                as.character(t2-t1),
                                                's, you can find the results in ', MVIDIA_DEA_out$zipfile))

    output$volc_MVIDIA <- renderPlot({
      MVIDIA_DEA_out$p_mv
    })
    output$upset_MVIDIA <- renderPlot({
      MVIDIA_DEA_out$p_us
    })

    # output$DEA_res_MVIDIA<-downloadHandler(
    #   filename <- function() {
    #     paste("DEA_res_MVIDIA", "zip", sep=".")
    #   },
    #
    #   content <- function(file) {
    #     file.copy(MVIDIA_DEA_out$zipfile, file)
    #
    #   },
    #   contentType = "application/zip"
    # )
  ...
  output$PD_result_MVIDIA<-renderUI({

    view_files_train<-input$view_dt_pd_tr
    view_files_test<-input$view_dt_pd_te

    platform = input$rb_pd_plat
    acq = input$rb_pd_acq
    view_names = paste(input$rb_pd, collapse = ',')
    pos = input$Pos
    signature = input$Signature
    method = input$rb_pd_mth
    #mr = input$mr
    python_path_mv_dea = ''#input$python_path_fg_ens

    t1 = proc.time()[3]
    MVIDIA_pd_out<-MVIDIA_class(view_files_train, view_files_test,
                               platform, acq, view_names, pos, signature, #mr,
                               python_path_mv_dea, method)
    #browser()
    t2 = proc.time()[3]
    output$PD_result_MVIDIA<-renderText(paste0('MVIDIA patient diagnosis finished with time ',
                                               as.character(t2-t1),
                                               's, you can find the results in ', MVIDIA_pd_out))

    # output$PD_res_MVIDIA<-downloadHandler(
    #   filename <- function() {
    #     paste("PD_res_MVIDIA", ".zip", sep=".")
    #   },
    #
    #   content <- function(file) {
    #     file.copy(MVIDIA_pd_out, file)
    #
    #   },
    #   contentType = "application/zip"
    # )
  ...
  output$UMAP_MVIDIA<-renderUI({

    view_files<-input$view_dt_cc
    platform = input$rb_sc_plat
    acq = input$rb_sc_acq
    view_names = paste(input$rb_sc, collapse = ',')
    nclus = input$nclus
    mr = input$mr
    ld = input$ld
    c = input$c
    ...
    t1 = proc.time()[3]
    MVIDIA_clustering_out<-MVIDIA_clustering(view_files, platform, acq, view_names,
                               nclus, mr, ld, c,
                               python_path_mv_dea)
    t2 = proc.time()[3]
    output$clustering_result_MVIDIA<-renderText(paste0('MVIDIA Clustering finished with time ',
                                                as.character(t2-t1),
                                                's, you can find the results in ', MVIDIA_clustering_out$zipfile))

    output$UMAP_MVIDIA <- renderPlot({
      MVIDIA_clustering_out$pall
    })
    # output$Clustering_res_MVIDIA<-downloadHandler(
    #   filename <- function() {
    #     paste("Clustering_res_MVIDIA", "zip", sep=".")
    #   },
    #
    #   content <- function(file) {
    #     file.copy(MVIDIA_clustering_out$zipfile, file)
    #
    #   },
    #   contentType = "application/zip"
    # )
    ...
    output$ext_fg<-renderUI({
      #browser()
      raw<-input$fg_cb_pro
      evid<-input$fg_cb_ion
      design<-input$fg_cb_dg
      exp = paste(input$rb_ext_fg, collapse = ',')

      tool = 'off'
      python_path_mv_dea = ''#input$python_path_fg_ens

      t1 = proc.time()[3]
      out_ext_fg_dda<-extract_fg_dda(raw, evid, design, exp, python_path, tool)
      #browser()
      t2 = proc.time()[3]
      output$ext_fg<-renderText(paste0('View extration finished with time ',
                                                 as.character(t2-t1),
                                                 's, you can find the view data at ', out_ext_fg_dda))

      # output$fg_ext_view<-downloadHandler(
      #   filename <- function() {
      #     paste("views", ".zip", sep=".")
      #   },
      #
      #   content <- function(file) {
      #     file.copy(out_ext_fg_dda, file)
      #
      #   },
      #   contentType = "application/zip"
      # )
    ...
    output$ext_mq<-renderUI({

      raw<-input$mq_pro
      evid<-input$mq_evid
      design<-input$mq_dg
      exp = paste(input$rb_ext_mq, collapse = ',')

      tool = 'off'
      python_path_mv_dea = ''#input$python_path_fg_ens

      t1 = proc.time()[3]
      out_ext_mq_dda<-extract_mq_dda(raw, evid, design, exp, python_path, tool)
      #browser()
      t2 = proc.time()[3]
      output$ext_mq<-renderText(paste0('View extration finished with time ',
                                       as.character(t2-t1),
                                       's, you can find the view data at ', out_ext_mq_dda))

      # output$mq_ext_view<-downloadHandler(
      #   filename <- function() {
      #     paste("views", ".zip", sep=".")
      #   },
      #
      #   content <- function(file) {
      #     file.copy(out_ext_mq_dda, file)
      #
      #   },
      #   contentType = "application/zip"
      # )
    ...
    output$ext_diann<-renderUI({

      raw<-'-'
      evid<-input$diann_rep
      design<-input$diann_dg
      exp = paste(input$rb_ext_dia, collapse = ',')

      tool = 'off'
      python_path_mv_dea = ''#input$python_path_fg_ens

      t1 = proc.time()[3]
      out_ext_diann_dia<-extract_diann_dia(raw, evid, design, exp, python_path, tool)
      #browser()
      t2 = proc.time()[3]
      output$ext_diann<-renderText(paste0('View extration finished with time ',
                                       as.character(t2-t1),
                                       's, you can find the view data at ', out_ext_diann_dia))

      # output$diann_ext_view<-downloadHandler(
      #   filename <- function() {
      #     paste("views", ".zip", sep=".")
      #   },
      #
      #   content <- function(file) {
      #     file.copy(out_ext_diann_dia, file)
      #
      #   },
      #   contentType = "application/zip"
      # )
    ...
    output$ext_spt<-renderUI({

      raw<-'-'
      evid<-input$spt_rep
      design<-input$spt_dg
      exp = paste(input$rb_ext_dia_spt, collapse = ',')

      tool = 'off'
      python_path_mv_dea = ''#input$python_path_fg_ens

      t1 = proc.time()[3]
      out_ext_spt_dia<-extract_spt_dia(raw, evid, design, exp, python_path, tool)
      #browser()
      t2 = proc.time()[3]
      output$ext_spt<-renderText(paste0('View extration finished with time ',
                                          as.character(t2-t1),
                                          's, you can find the view data at ', out_ext_spt_dia))

      # output$spt_ext_view<-downloadHandler(
      #   filename <- function() {
      #     paste("views", ".zip", sep=".")
      #   },
      #
      #   content <- function(file) {
      #     file.copy(out_ext_spt_dia, file)
      #
      #   },
      #   contentType = "application/zip"
      # )
  ...
  # output$fg_exam_raw <- downloadHandler(
  #   filename <- function() {
  #     paste("Example_raw_quantification", "zip", sep=".")
  #   },
  #
  #   content <- function(file) {
  #     file.copy('/home/ubuntu/shiny_data/www/example/view_preparation.R', file)
  #
  #   },
  #   contentType = "application/zip"
  # )
