...
def main():
    ...
    parser.add_argument(python'-A', '--acq', default='DDA', required=True,
                        help='result saving folder')
...
if __name__ == '__main__':
    ...
    R_fold = ''
    save_folder = save_data_root + 'raw_res_3v_dis_0_h/'

    view_str = 'dlfq,maxlfq,top3'

    mat_folder = root_fold + 'Supplementary_codes/Result_reproducing/Patient_diagnosis/data/raw_'
    design_path = root_fold + 'Supplementary_codes/Result_reproducing/Patient_diagnosis/data/raw_avg_design_dis.csv'
    ...
    design_path_tr = mat_folder + 'design_path_tr.csv'
    pd.DataFrame(design_value, columns=design_headers).to_csv(design_path_tr, sep='\t', index=False)

    view_files = mat_folder + 'avg_dlfq_dis.csv,' + mat_folder + 'avg_maxlfq_dis.csv,' + mat_folder + 'avg_top3_dis.csv'
